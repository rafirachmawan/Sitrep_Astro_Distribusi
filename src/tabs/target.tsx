...
type ChecklistOverrides = {
  ...
  // rows untuk base & custom section
...
function toTidyChecklistRows(
  ...
  Object.keys(FINAL_MAP).forEach((sec) => {
    ...
    FINAL_MAP[sec].rows.forEach((def) => {
      const v = (data as any)?.[sec]?.[def.key] as RowValue | undefined;
      if (!v) {
        rows.push({
          sectionKey: String(sec),
...
export default function ChecklistArea({
  ...
  // ===== Superadmin Section Handlers (base & custom) =====
  ...
  // >>> NEW: Toggle hidden base/custom (hilang di versi sebelumnya)
  const toggleSectionHiddenAny = (sec: AnySectionKey, hidden: boolean) => {
    if (!isSuper) return;
    const cur = readRoleOverrides(viewRole);

    if (String(sec).startsWith("x_")) {
      const key = sec as `x_${string}`;
      const prev = cur.extraSections?.[key] || {
        title: FINAL_MAP[sec]?.title || String(sec),
      };
      const next = mergeExtraSection(cur, key, { ...prev, hidden });
      writeRoleOverrides(viewRole, next);
      setRev((x) => x + 1);
      return;
    }
  ...
  }, [overrides, secActive, overrides.extraSections, overrides.sections]);
  ...
  return (
    <div className="bg-white border rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="px-3 sm:px-6 pb-4">
        ...
        {/* Panel besar (tetap ada, hanya untuk superadmin edit) */}
        {isSuper && editMode && (
